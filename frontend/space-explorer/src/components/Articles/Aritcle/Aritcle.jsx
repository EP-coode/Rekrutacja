...
function Article({ article, liked = false }) {
    console.log(article);
    // nieładnie, ale tymczasowo może być

    return (
        <li className="space-article" key={article.id}>
            <a className="space-article__banner" href={article.url} target="_blank">
                <h3 className="space-article__title">{article.title}</h3>
                <span className="space-article__source">{article.newsSite}</span>
            ...
            <div className="space-article__details">
                ...
                <div className="space-article__foot">
                    ...
                    <button className="space-article__like-btn">
                        {!liked ? <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" fill="currentColor" class="bi bi-heart" viewBox="0 0 16 16">
                    ...
                    <span className="space-article__timestamp">{new Date(article.publishedAt).toLocaleString()}</span>
                </div>
            </div>
        </li>
    );
